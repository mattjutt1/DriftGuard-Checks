...
from tests.fixtures.sample_prompts import SAMPLE_PROMPTS, BATCH_TEST_DATA
...
class TestCompleteWorkflows:
    ...
    def test_health_check_to_optimization_workflow(self, cli_runner):
        ...
        # Step 2: Optimization
        opt_result = cli_runner.invoke(cli, [
            'optimize', 
...
class TestPerformanceWorkflows:
    ...
    def test_quick_vs_advanced_mode_timing(self, cli_runner):
        ...
        responses.add_callback(
            responses.POST,
            f"{CONVEX_BASE_URL}/optimize",
            callback=quick_response_callback
        )
        
        # Test quick mode
        start_time = time.time()
        quick_result = cli_runner.invoke(cli, [
            'optimize', 'Test prompt for timing',
            '--mode', 'quick'
        ])
        ...
        responses.add_callback(
            responses.POST,
            f"{CONVEX_BASE_URL}/optimize",
            callback=advanced_response_callback
        )
        
        # Test advanced mode
        start_time = time.time()
        advanced_result = cli_runner.invoke(cli, [
            'optimize', 'Test prompt for timing',
            '--mode', 'advanced'
        ])
