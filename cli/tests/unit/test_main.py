...
from promptevolver_cli.main import cli, _display_quality_metrics, _save_optimization_results
from promptevolver_cli.client import ConvexError
from tests.fixtures.sample_prompts import SAMPLE_PROMPTS, CONFIG_TEST_DATA
...
class TestOptimizeCommand:
    ...
    def test_optimize_with_file_input(self, mock_client, cli_runner, temp_prompt_file):
        ...
        mock_client.optimize_prompt.return_value = {
            "success": True,
            "result": {
                "best_prompt": "File-based optimized prompt",
                "quality_score": 90.0
            }
        }
    ...
    def test_optimize_with_comparison(self, mock_client, cli_runner):
        ...
        mock_client.optimize_prompt.return_value = {
            "success": True,
            "result": {
                "best_prompt": "Optimized version",
                "quality_score": 90.0
            }
        }
        
        result = cli_runner.invoke(cli, [
            'optimize', 'Original prompt', '--show-comparison'
        ])
        assert result.exit_code == 0
...
class TestBatchCommand:
    ...
    def test_batch_processing_success(self, mock_client, cli_runner, temp_prompt_file, temp_output_dir):
        ...
        mock_client.optimize_prompt.return_value = {
            "success": True,
            "result": {
                "best_prompt": "Batch optimized prompt",
                "quality_score": 85.0
            }
        }
    ...
    def test_batch_processing_with_errors(self, mock_client, cli_runner, temp_prompt_file):
        ...
        result = cli_runner.invoke(cli, [
            'batch', str(temp_prompt_file), '--continue-on-error'
        ])
