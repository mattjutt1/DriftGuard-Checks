...
class PromptWizardVerifier:
            ...
            try:
                with open(file_path, 'r') as f:
                    ...
                    if sample_data:
                        ...
                        is_real = "+" in sample_data["full_solution"] or \
                                 "-" in sample_data["full_solution"] or \
                                 "*" in sample_data["full_solution"] or \
    ...
    def verify_promptwizard_framework(self) -> bool:
        ...
        for file_path in critical_files:
            ...
            self.print_result(
                f"PromptWizard {file_path.split('/')[-1]}",
                exists,
        ...
        core_logic = pw_dir / "promptwizard/glue/promptopt/techniques/critique_n_refine/core_logic.py"
        ...
        for url, description, required in endpoints:
            try:
                ...
                self.print_result(
                    description,
                    not required,
                    "Connection error (may be normal)"
                )
                if required:
                    all_passed = False
            except Exception as e:
                self.print_result(
                    description,
                    not required,
                    f"Not accessible: {type(e).__name__}"
                )
                if required:
                    all_passed = False
    ...
    def verify_build_status(self) -> bool:
        ...
        try:
            ...
            build_success = "Generating static pages" in result.stdout and \
    ...
    def generate_report(self):
        ...
        all_passed = all(
            v for v in self.results.values() 
            if v is not None
        )
