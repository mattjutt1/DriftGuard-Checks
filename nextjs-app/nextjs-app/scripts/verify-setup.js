...
if (fs.existsSync(envPath)) {
  ...
  requiredVars.forEach(varName => {
    const hasVar = envContent.includes(`${varName}=`) && 
                   ...
                   !envContent.includes(`${varName}=`);
    console.log(`${envContent.includes(varName) && !envContent.match(`${varName}=\\s*$`) ? '✅' : '❌'} ${varName}`);
  });
} else {
...
if (fs.existsSync(gitignorePath)) {
  ...
  console.log(`${fs.existsSync(path.join(__dirname, '..', '.env.example')) ? '✅' : '❌'} .env.example template provided`);
} else {
