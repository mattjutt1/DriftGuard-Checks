...
Provide an enhanced version that is clearer and more effective."""
        ...
        with torch.no_grad():
            outputs = model.generate(
                **inputs, temperature=temperature, max_new_tokens=512, do_sample=True, top_p=0.95
            )
...
with gr.Blocks(title="PromptEvolver") as demo:
    ...
    **Status**: {"Ready" if MODEL_LOADED else "Running in test mode"}
    ...
    with gr.Row():
        with gr.Column():
            prompt_input = gr.Textbox(
                label="Original Prompt", placeholder="Enter your prompt to optimize...", lines=4
            )
            ...
            with gr.Row():
                mode = gr.Radio(["quick", "balanced", "thorough"], value="balanced", label="Mode")
    ...
    optimize_btn.click(
        fn=optimize_prompt, inputs=[prompt_input, task_input, mode, temp], outputs=output
    )
