...
class ContextVectorUpdater:
    ...
    def calculate_quality_metrics(self, graph, embeddings):
        ...
        embedded_files = len([e for e in embeddings.get("embeddings", {}) 
        ...
        embedded_agents = len([e for e in embeddings.get("embeddings", {}) 
    ...
    def generate_context_summary(self, session_context, patterns, metrics, avsha_data=None):
        ...
        if avsha_data:
            ...
            if avsha_metrics.get("architectural_compliance", 0.0) < 0.8:
                summary["recommendations"].append("AVSHA: Improve architectural compliance - structure files according to atomic levels and feature slices")
            
            if avsha_metrics.get("reusability_score", 0.0) < 0.3:
                summary["recommendations"].append("AVSHA: Increase component reusability - move common components to /shared/")
            ...
            if component_dist.get("atoms", 0) < 3:
                summary["recommendations"].append("AVSHA: Consider creating more atomic components for better reusability")
            
            if len(avsha_data.get("feature_slices", {})) < 3:
                summary["recommendations"].append("AVSHA: Organize code into feature slices (authentication, optimization, dashboard)")
        ...
        context_vectors = {
            ...
            "claude_optimization": {
                "high_priority_context": [
                    ...
                    f"Active agents: {', '.join([a['name'] for a in session_context['active_agents']])}" if session_context['active_agents'] else "No active agents",
                    f"Recent files: {len(session_context['recent_files'])} modified",
                    f"Frameworks: {', '.join(patterns['frameworks'])}" if patterns['frameworks'] else "No frameworks detected",
